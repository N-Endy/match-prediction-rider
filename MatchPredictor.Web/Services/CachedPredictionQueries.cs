...
public class CachedPredictionQueries : IPredictionQueries
{
    ...
    public Task<IReadOnlyList<Prediction>> GetRecentSettledPublishedAsync(int days = 30)
    {
        var windowDays = Math.Clamp(days, 1, 90);
        return GetOrCreateAsync(
            $"predictions:settled-published:{windowDays}",
            () => _inner.GetRecentSettledPublishedAsync(windowDays));
    }
